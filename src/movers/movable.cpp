...
movable::movable(std::string path) :
  ...
  last_pos(0,0),
  vel(0,0),
  vel_accel(xmlparse::get().get_xml_float(path + "/movement/vel_accel") * scale_factor * t_frame::get().t_factor()),
  vel_accel_mod(1),
  vel_cap(xmlparse::get().get_xml_float(path + "/movement/vel_cap") * scale_factor * t_frame::get().t_factor()),
  vel_cap_mod(1),
//  vel_overcap(xmlparse::get().safe_get_xml_int(path + "/movement/vel_overcap")),
  vel_decay(xmlparse::get().get_xml_float(path + "/movement/vel_decay") * scale_factor * t_frame::get().t_factor()),
  moved(false),
