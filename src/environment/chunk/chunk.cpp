...
void chunk::draw_nc() const {
  if(!in_bounds) { return; }
  image_handler::get().nc_draw_bg(tlc, {tlc[0] + length, tlc[1] + length}, 100000);

  if(border[0]) { /*top border*/ 
    image_handler::get().nc_draw_line(tlc, {tlc[0] + length, tlc[1]}, 'N'); 
  }
  if(border[1]) { /*bottom border*/ 
    image_handler::get().nc_draw_line({tlc[0], tlc[1] + length}, {tlc[0] + length, tlc[1] + length}, 'N'); 
  }
  if(border[2]) { /*left border*/ 
    image_handler::get().nc_draw_line(tlc, {tlc[0], tlc[1] + length}, 'N'); 
  }
  if(border[3]) { /*right border*/ 
    image_handler::get().nc_draw_line({tlc[0] + length, tlc[1]}, {tlc[0] + length, tlc[1] + length}, 'N');
  }
}
